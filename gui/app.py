...
# times = filemap.select(pl.col("Time")).unique().to_numpy().squeeze().tolist()
# points = filemap.select(pl.col("Point")).unique().to_numpy().squeeze().tolist()
...
# ):
#     column_at_ecdysis = [f"{column}_at_{e}" for e in ECDYSIS_COLUMNS]
#     ecdysis_df = filemap.select(pl.col(column_at_ecdysis + ["Point"]))
#     values_at_ecdysis = (
#         (
#             ecdysis_df.groupby("Point")
#             .agg(pl.col(column_at_ecdysis).first())
#             .select(pl.col(column_at_ecdysis))
#         )
#         .to_numpy()
#         .squeeze()
#     )
...
#         )
#     feature_at_ecdys_columns = []
#     for feature_column in feature_columns:
#         for ecdys in ECDYSIS_COLUMNS:
#             feature_at_ecdys_column = f"{feature_column}_at_{ecdys}"
#             feature_at_ecdys_columns.append(feature_at_ecdys_column)
#             if feature_at_ecdys_column not in columns:
#                 filemap.with_columns(pl.lit(np.nan).alias(feature_at_ecdys_column))
...
#         filemap.loc[
#             filemap["Point"] == point, [feature_at_ecdysis_column]
#         ] = compute_series_at_time_classified(
#             data_of_point[feature_column].values,
#             data_of_point[worm_type_column].values,
#             data_of_point[ecdys_event].values[0],
