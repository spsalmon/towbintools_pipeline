...
To access the GUI, first go to the GUI directory and run the commands :

```bash
cd ~/towbintools_pipeline/gui
micromamba activate towbintools
shiny run -p0
```

To specify which experiement you want to open, change the **filemap_path** variable in 'gui/app.py' to the path of the filemap you want to open.
When opening a filemap in the GUI, we automatically create a backup in the same folder. The GUI then saves the annotated version of the filemap with an "_annotated" suffix. By default, the GUI always tries to load the annotated version if it exists. You can disable this behavior by switching **open_annotated** to False in 'gui/app.py'. If False, it will always open exactly the filemap path provided.
