...
def segment_and_save_ilastik(image_path, output_path, ilastik_project_path, augment_contrast=False, clip_limit=5, channels=[], is_zstack=False, result_channel=0):
    ...
    if (image.ndim > 2) and (is_zstack == False):
        raise ValueError('The image is not a z-stack, but has more than 2 dimensions. Ilastik only works on 2D single channel images.')
...
def main(input_pickle, output_pickle, config, n_jobs):
    ...
    if config['segmentation_method'] == 'edge_based':
        ...
        if len(devices) == 1:
            ...
            modelz = [models.LightningPretrained.load_from_checkpoint(config['model_path'], map_location=device) for device in devices]
            ...
            def process_on_gpu(input_data, output_path, gpu_id):
                ...
                model = modelz[gpu_id]
                return segment_and_save(input_data, output_path, method=config['segmentation_method'], augment_contrast=config['augment_contrast'], channels=config[
                    'segmentation_channels'], model=model, tiler=tiler, RGB=config['RGB'], preprocessing_fn=preprocessing_fn,activation=config['activation_layer'], device = device, batch_size=config['batch_size'], is_zstack=is_zstack)
            
            Parallel(n_jobs=device_count)(delayed(process_on_gpu)(input_file, output_path, i%device_count) for i, (input_file, output_path) in enumerate(zip(input_files, output_files)))
