...
def compute_other_features_at_molt(analysis_filemap, molt_dataframe, volume_column, worm_type_column, point):
    ...
    volumes_to_compute = [column for column in data_of_point.columns if ('volume' in column and 'VolumeAt' not in column and column != volume_column)]
    lengths_to_compute = [column for column in data_of_point.columns if ('length' in column)]
    areas_to_compute = [column for column in data_of_point.columns if ('area' in column)]
...
def main(input_dataframe_path, output_file, config, n_jobs):
    ...
    other_features_at_molt = Parallel(n_jobs=n_jobs)(delayed(compute_other_features_at_molt)(
        analysis_filemap, molts_dataframe, config['molt_detection_volume'], config['molt_detection_worm_type'], point) for point in analysis_filemap['Point'].unique())
